...
N = 100;
[X,Y,Z] = peaks(N); %a pre-defined function (complicated)

% get gradient
[dfdx,dfdy] = gradient(Z,X(1,2)-X(1,1),Y(2,1)-Y(1,1));

% plot function
s=surf(X,Y,Z); %3-D surface plot
%s.EdgeColor = 'none';
hold on
xlabel('x'); ylabel('y');
% plot gradient
quiver(X,Y,dfdx,dfdy,'k') %give direction in which the gradient is moving
...
xlabel('x'); ylabel('y'); %direction in which gradient moves. It should move opposite to the arrows. 
...
%==================================================
% constraint function being the circle
...
% plot gradient along circle (important)
figure
contour(X,Y,Z); hold on
plot(x,y,'k');
