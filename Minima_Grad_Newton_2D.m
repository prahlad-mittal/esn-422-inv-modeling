...
d2fdx2 =@(X,Y) 12*cosd(X)*sind(X)-4*(cosd(X).^4)-4*(cosd(X).^2)*(cosd(Y).^2)+4*(cosd(Y).^2)*(sind(X).^2);
d2fdy2 = @(X,Y)12*cosd(Y)*sind(Y)-4*(cosd(Y).^4)-4*(cosd(X).^2)*(cosd(Y).^2)+4*(cosd(X).^2)*(sind(Y).^2);
d2fdxdy = @(X,Y)8*cosd(X)*sind(X)*cosd(Y)*sind(Y);
d2fdydx =@(X,Y) 8*cosd(X)*sind(X)*cosd(Y)*sind(Y);
